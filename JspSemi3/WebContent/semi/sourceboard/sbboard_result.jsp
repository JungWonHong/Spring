...
%>
	<tr align=center height=20>
		<td colspan=7>
			...
			[이전]&nbsp;
			<%}else{ %>
			<a href="./sbBoardSearchAction.bo?page=<%=nowpage-1 %>&keyField=<%=keyField%>&keyWord=<%=keyWord%>">[이전]</a>&nbsp;
			<%} %>
			...
			<% if(nowpage>=maxpage){%>
				[다음]
			<% }else{ %>
			<a href="./sbBoardSearchAction.bo?page=<%=nowpage+1 %>&keyField=<%=keyField%>&keyWord=<%=keyWord%>'">[다음]</a>&nbsp;	
			<%} %>
		</td>
	</tr>
