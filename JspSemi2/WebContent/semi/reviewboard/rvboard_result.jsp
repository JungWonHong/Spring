...
<table width=570 border="0" cellpadding="0" cellspacing="0">
	<tr align="center" valign="middle">
		<td colspan="4">현장의 소리 게시판(검색 결과창)</td>
		<td align=right>
...
%>
	<tr align=center height=20>
		<td colspan=7>
			...
			[이전]&nbsp;
			<%}else{ %>
			<a href="./rvBoardSearchAction.bo?page=<%=nowpage-1 %>&keyField=<%=keyField%>&keyWord=<%=keyWord%>">[이전]</a>&nbsp;
			<%} %>
			...
			<%for(int a=startpage; a<=endpage; a++){ 
				...
				<%}else{ %>
				<a href="./rvBoardSearchAction.bo?page=<%=a %>&keyField=<%=keyField%>&keyWord=<%=keyWord%>">[<%=a %>]</a>&nbsp;
				<%} %>
			<%} %>
			<% if(nowpage>=maxpage){%>
				[다음]
			<% }else{ %>
			<a href="./rvBoardSearchAction.bo?page=<%=nowpage+1 %>&keyField=<%=keyField%>&keyWord=<%=keyWord%>'">[다음]</a>&nbsp;	
			<%} %>
		</td>
	</tr>
