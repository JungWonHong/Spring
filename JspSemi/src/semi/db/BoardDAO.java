...
public class BoardDAO {
	...
	public List<EListBean> getEList() {
		String sql = "select * from E_LIST";
		List<EListBean> list = new ArrayList<EListBean>();
		try{
			con = ds.getConnection();
			pstmt = con.prepareStatement(sql);
			rs = pstmt.executeQuery();
			...
			while(rs.next()){
				EListBean e = new EListBean();
				e.setEl_company(rs.getString("El_company"));
				e.setEl_environment(rs.getString("El_environment"));
				e.setEl_forms(rs.getInt("El_forms"));
				e.setEl_head(rs.getString("El_head"));
				e.setEl_loc(rs.getString("El_loc"));
				e.setEl_part(rs.getString("El_part"));
				e.setEl_workernum(rs.getInt("El_workernum"));
				list.add(e);
			}
			
			return list;
		} catch (Exception ex) {
			System.out.println("getEList() 에러" + ex);
		} finally {
			if (rs != null)
				try {
					rs.close();
