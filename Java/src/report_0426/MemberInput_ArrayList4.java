...
public class MemberInput_ArrayList4 {
	...
	static int titlePrint(String str, Scanner sc) {
		int search;
		do {
			System.out.println("=== " + str + "할 항목을 선택하세요 ===");
	...
	static void search(ArrayList<MemberInfo> list, Scanner sc) {
		int selectItem = titlePrint("검색", sc);
	...
	static void update(ArrayList<MemberInfo> list, Scanner sc) {
		int selectItem = titlePrint("수정", sc);
		String[] itemname = { "성명", "나이", "E-Mail", "주소" };
		...
		int count = 0;
		ArrayList<MemberInfo> updateList = new ArrayList<MemberInfo>();
		for (MemberInfo a : list) {
			count = search_each(a, searchdata, selectItem);
			updateList.add(a);
		}
		if (count == 0) {
