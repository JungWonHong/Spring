...
public class MemberInput_method2 {
	...
	static void delete(ArrayList<MemberInfo> list, Scanner sc) {
		int selectItem = titlePrint(sc, 4);
		String[] itemname = { "성명", "나이", "E-Mail", "주소" };
		System.out.println("삭제 할 " + itemname[selectItem - 1] + "을(를) 입력하세요.");
		...
		for (int i = list.size() - 1; i >= 0; i--) {
			MemberInfo a = list.get(i);
			int ok = search_each(a, del, selectItem);
			if (ok != 0) {
				list.remove(a);
				print(a);
				count++;
		...
		System.out.println("===" + count + "개의 항목을 성공적으로 삭제했습니다.===");
		if (count == 0)
